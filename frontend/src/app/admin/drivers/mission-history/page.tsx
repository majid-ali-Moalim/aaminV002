...
export default function DriverMissionHistoryPage() {
  ...
  const history = useMemo(
    () =>
      requests
        .filter((r) => r.driverId && HISTORY_STATUSES.includes(r.status))
    ...
    <div className="p-6 max-w-[1600px] mx-auto space-y-6 pb-12">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-red-100/80 mt-2 max-w-xl text-sm">
              Past emergency missions with assigned drivers — completed, hospital arrivals, and cancelled.
            </p>
          </div>
          <Button
            variant="outline"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Completed', value: stats.completed, icon: Truck, color: 'text-emerald-600 bg-emerald-50' },
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search tracking code, driver, location, or unit…"
            value={searchTerm}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  ...
                  <th className="text-left px-5 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Driver</th>
                  ...
                  filtered.map((req) => {
                    ...
                    return (
                      <tr key={req.id} className="hover:bg-red-50/30 transition-colors">
                        ...
                        <td className="px-5 py-4 font-medium text-slate-700">{driverName}</td>
