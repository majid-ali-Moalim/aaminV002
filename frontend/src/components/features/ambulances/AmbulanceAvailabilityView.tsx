...
export default function AmbulanceAvailabilityView() {
  ...
  const summary = data?.summary ?? { total: 0, available: 0, unavailable: 0, maintenance: 0, activeToday: 0 }
  ...
  return (
    <div className="p-6 max-w-[1800px] mx-auto space-y-6 pb-12 print:p-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <KpiCard label="Maintenance" value={summary.maintenance} icon={Wrench} tone="amber" />
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 space-y-3 print:hidden">
        <div className="flex flex-col lg:flex-row gap-3">
          ...
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as StatusFilterTab)} className="lg:w-44 px-3 py-2.5 rounded-xl border border-slate-200 text-sm">
            {STATUS_TABS.map((tab) => (
...
function KpiCard({ label, value, icon: Icon, tone }: { label: string; value: number; icon: typeof Truck; tone: 'slate' | 'emerald' | 'red' | 'blue' | 'violet' | 'amber' }) {
  const colors = { slate: 'text-slate-600 bg-slate-50', emerald: 'text-emerald-600 bg-emerald-50', red: 'text-red-600 bg-red-50', blue: 'text-blue-600 bg-blue-50', violet: 'text-violet-600 bg-violet-50', amber: 'text-amber-600 bg-amber-50' }
  return (
