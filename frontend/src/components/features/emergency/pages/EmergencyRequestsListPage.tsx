...
export default function EmergencyRequestsPage() {
  ...
  const filteredRequests = periodRequests.filter(request => {
    ...
    <div className="bg-slate-50 min-h-screen font-sans">
      ...
      <header className="bg-white border-b border-slate-200 px-8 py-4 flex items-center justify-between sticky top-0 z-50 shadow-sm">
        ...
        <div className="flex items-center gap-3">
           ...
           <Link href={paths.patientsCases}>
             <Button
               variant="outline"
               className="h-11 px-5 border-slate-200 text-slate-700 hover:bg-slate-50 font-bold text-sm rounded-xl"
             >
               Completed cases
             </Button>
           </Link>
      ...
      <main className="p-8 max-w-[1600px] mx-auto space-y-8">
        ...
        {/* Date filter — drives stats and table below */}
        <EmergencyDateFilter
          preset={datePreset}
        ...
        <div>
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4">
            Cases in period: <span className="text-red-600">{dateRange.label}</span>
          </p>
