...
import { Stethoscope, Settings, X, Plus, Save, Activity, CheckCircle2, Loader2 } from 'lucide-react'
import { cn } from '@/lib/utils'

interface EquipmentLevelFormProps {
  initialData?: {
    id?: string
    name?: string
    description?: string
...
export default function EquipmentLevelForm({ initialData, onSubmit, onCancel, loading }: EquipmentLevelFormProps) {
        ...
        <div className="relative flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-white/15 rounded-xl">
              <Stethoscope className="w-5 h-5 text-red-100" />
            </div>
            <div>
              <h2 className="text-lg font-black tracking-tight leading-tight">
                {isEditing ? 'Edit Level' : 'New Equipment Level'}
              </h2>
              <p className="text-red-200/70 text-xs font-medium">
                {isEditing ? 'Update equipment standards' : 'Define a new medical equipment level'}
              </p>
            </div>
          </div>
          <button
      ...
      <form onSubmit={handleSubmit} noValidate className="px-6 py-5 space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <input
            type="text"
            required
            placeholder="e.g. Basic, Advanced, Critical Care"
            value={formData.name}
            onChange={(e) => {
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            rows={3}
            placeholder="Brief details about the level's capacity..."
        ...
        <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Level is {formData.isActive ? 'active in the system' : 'archived / hidden'}</p>
            </div>
          </div>
        ...
        <div className="flex gap-3">
          ...
          >
            {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : isEditing ? 'Update Level' : 'Create Level'}
          </button>
        </div>
      </form>
    </div>
