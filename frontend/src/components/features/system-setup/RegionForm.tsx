...
import { MapPin, Settings, X, Plus, Save, Activity, CheckCircle2, Loader2 } from 'lucide-react'
...
export default function RegionForm({ initialData, onSubmit, onCancel, loading }: RegionFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.code.trim()) {
      ...
      dataToSubmit.code = null // Send null to safely clear/omit it for the unique constraint
    }
        ...
        <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Region is {formData.isActive ? 'visible in the system' : 'archived / hidden'}</p>
            </div>
          </div>
