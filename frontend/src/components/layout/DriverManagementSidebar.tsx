...
interface DriverManagementSidebarProps {
  isOpen: boolean
  setOpen: (open: boolean) => void
}

export default function DriverManagementSidebar({ isOpen, setOpen }: DriverManagementSidebarProps) {
  const pathname = usePathname()
  const isSectionActive = isDriverManagementPath(pathname)
  ...
  return (
    <div className="rounded-lg overflow-hidden">
      ...
      {isOpen && (
        ...
        >
          {driverMenuItems.map((item) => renderItem(item))}
        </div>
      )}
    </div>
  )
