...
export function isPatientsCaseRecordsPath(pathname: string) {
  return pathname.startsWith('/admin/patients')
}

interface PatientsCaseRecordsSidebarProps {
  isOpen: boolean
  setOpen: (open: boolean) => void
}

export default function PatientsCaseRecordsSidebar({ isOpen, setOpen }: PatientsCaseRecordsSidebarProps) {
  const pathname = usePathname()
  const isSectionActive = isPatientsCaseRecordsPath(pathname)
    ...
    <div className="rounded-lg overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          />
          <span className="truncate">Patients & Case Records</span>
        </div>
        {isOpen ? (
          <ChevronDown
            className="w-4 h-4 shrink-0"
      ...
      {isOpen && (
        ...
        >
          {patientsMenuItems.map((item) => renderItem(item))}
        </div>
      )}
    </div>
  )
